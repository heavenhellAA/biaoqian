...
{
    public class YinfeiteController : Controller
    {
        //
        // GET: /Yinfeite/
        ...
        public int AddYinfeitePrintZX()
        {
            ...
            string codemsg = "A020063" + '#' + khlh + '#' + scrq;
            ...
            sql = "declare @count varchar(100)    set @count=1 While(convert(int,@count) <= '" + num_print + "') begin Insert Into tbYinfeitePrint(s_id,s_packageType,s_khlh,s_sl,s_scrq,s_ggxh,s_taitou,s_creator,s_createtime,s_Groupid,s_Roleid,s_waternum,s_cnbqywm,s_codemsg,n_state,n_bdprint) ";
            sql += "values(NEWID(),'" + packageType + "','" + khlh + "','" + sl + "','" + scrq + "','" + ggxh + "','" + taitou + "'";
        ...
        public int Zuofei(string delstr)
        {
            ...
            if (delstr != "")
            {
                ...
                string nowtime = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
                string sql = "update tbYinfeitePrint set n_state = 1 ,s_updator='" + UserID + "',s_updatetime='" + nowtime + "' where n_id in (" + delstr + ")";
                res = DBHelper.excuteNoQuery(sql);
            }
            return res;
        }
        public int UpdateYinfeitePrint_BDZX()
        {
            int code = 0;
            string sql;
            ...
            string UserID = Server.HtmlDecode(Request.Cookies["bcp_userInfo"]["UserID"].ToString());
            string nowtime = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
            sql = "update tbYinfeitePrint set n_bdprint = 1 ,s_updator='" + UserID + "',s_updatetime='" + nowtime + "' where n_id = '" + id + "'";
            code = DBHelper.excuteNoQuery(sql);
            return code;
        }
    }
