...
{
    public class ZhongdaController : Controller
    {
        //
        // GET: /Zhongda/
        ...
        public string searchprint()
        {
            ...
            sql = "select tu.s_UserName,tg.s_GroupName,tr.s_RoleName,a.* from (select *,row_number() over (order by n_id DESC) as id from tbZhongdaPrint where n_state = 0) a left join tbuser tu  on  tu.s_UserID=a.s_creator left join tbGroup tg  on  tg.s_GroupID=a.s_Groupid left join tbRole tr  on  tr.s_RoleID=a.s_Roleid where id between ('" + limit_sql + "'*('" + page_sql + "'-1)+1) and '" + limit_sql + "'*('" + page_sql + "'-1)+'" + limit_sql + "' order by id  ";
            sql += "select COUNT(n_id)  as zongshu from tbZhongdaPrint where n_state = 0";
        ...
        public string Getlsnum(string scrq)
        {
            ...
            string sql;
            sql = " declare @lsnum nvarchar(20)   if not exists (select n_id from tbZhongdaPrint where n_state = 0 and '" + scrq + "' = s_lsh1)  set @lsnum = '000001' ";
            sql += "else  if exists (select n_id from tbZhongdaPrint where n_state = 0 and '" + scrq + "' = s_lsh1) set @lsnum = ( ";
            sql += "case when  (select top 1 right(s_lsh,6) as lsnum from tbZhongdaPrint where n_state=0 and '" + scrq + "' = s_lsh1 order by n_id DESC )  = '999999' then  '000001' ";
            sql += "else substring(convert(varchar,convert(int,'000001')+('1'+(select top 1 right(s_lsh,6) as lsnum from tbZhongdaPrint where n_state=0 and '" + scrq + "' = s_lsh1 order by n_id DESC ))),2,6) end )";
        ...
        public string AddZhongdaPrint()
        {
            ...
            sql = "Insert Into tbZhongdaPrint(s_id,s_cnbqywm,s_packageType,s_khlh,s_sl,s_dw,s_gysdm,s_zq,s_scph,s_mydm,s_ddh,s_fph,s_lsh1,s_lsh,s_cb,s_yxq,s_codemsg,s_creator,s_createtime,s_Groupid,s_Roleid,s_waternum,n_state,n_bdprint) ";
        ...
        public int UpdateZhongdaPrint()
        {
            ...
            string nowtime = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
            sql = "update tbZhongdaPrint set n_bdprint = 1 ,s_updator='" + UserID + "',s_updatetime='" + nowtime + "' where n_id = '" + id + "'";
            code = DBHelper.excuteNoQuery(sql);
            return code;
        }
        public int Zuofei(string delstr)
        {
            ...
            if (delstr != "")
            {
                ...
                string nowtime = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
                string sql = "update tbZhongdaPrint set n_state = 1 ,s_updator='" + UserID + "',s_updatetime='" + nowtime + "' where n_id in (" + delstr + ")";
